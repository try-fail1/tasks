...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    return questions.filter(
        (q: Question): boolean =>
            (q.body.length | q.expected.length | q.options.length) !== 0
    );
}
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (sum: number, curr: Question): number => sum + curr.points,
        0
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (sum: number, curr: Question): number =>
            curr.published ? sum + curr.points : sum,
        0
    );
}
...
export function sameType(questions: Question[]): boolean {
    return questions.length
        ? questions.every((q: Question) => q.type === questions[0].type)
        : true;
}
...
export function editOption(
    ...
    const element: Question | undefined = newQuestions.find(
    ...
    element.options.splice(
        targetOptionIndex === -1 ? element.options.length : targetOptionIndex,
        targetOptionIndex === -1 ? 0 : 1,
        newOption
    );
    return newQuestions;
}
...
export function duplicateQuestionInArray(
    ...
    const element: number | undefined = newQuestions.findIndex(
        (q: Question) => q.id === targetId
    );
    if (element === undefined) {
        return questions;
    ...
    newQuestions.splice(
        element + 1,
        0,
        duplicateQuestion(newId, newQuestions[element])
    );
    return newQuestions;
}
